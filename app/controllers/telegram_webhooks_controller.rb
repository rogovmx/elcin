class TelegramWebhooksController < Telegram::Bot::UpdatesController
  ...
  NY = ['Неважно где ты будешь встречать Новый год. Важно - с кем.',
        'Под Новый год обязательно поверьте в чудо - и оно обязательно произойдёт.',
        'Праздника чем больше, тем лучше.',
        'Новый год имеет запах, это запах мандарина и ёлки.',
        ...
        'В связи с наступлением Нового года в стране проходит всеобщая мандаринизация.',
        'В Новый год я видел счастливых людей - и трезвых среди них не было.',
        'Скоро Новый год... А мне еще до сих пор за прошлый стыдно...',
        'С каждой съеденной мандаринкой, вы приближаете Новый Год на 3 минуты и повышаете свой иммунитет!',
        'По-моему это кощунство: убить елку, а потом ходить вокруг трупа и петь "В лесу родилась ёлочка"!',
        'Новый год уже скоро, так что пора забывать о старых ошибках и планировать новые.',
        'Меня все спрашивают, что я насчёт нового года решил. Я решил ― пусть наступает!',
        'Кто не курит и не пьёт, тот запомнит Новый год.',
        'Одно чудо в новогоднюю ночь все-таки происходит постоянно… Холодильник на несколько дней становится безразмерным!'].freeze
  ...
  def message(message)
    ...
    unless words.nil?
      unless (words & ['нг', 'год', 'годом', 'году']).empty?
        sleep rand(5..20)
        respond_with :message, text: NY[rand(0..(NY.size-1))]
      end
